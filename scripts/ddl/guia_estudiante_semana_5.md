...
|---|---|---|
| 1 | `INSERT` válido en `cientifico_datos` y `proyecto` | Ninguno |
| 2 | `INSERT` en `experimento` con `id_proyecto = 999` (no existe) | va a salir un error indicando que el id que intenta asociar no existe |
| 3 | `INSERT` válido en `experimento` y `modelo` |  |
| 4 | `INSERT` en `metrica` con `valor = 1.5` (fuera de rango) | va a indicar que el valor que intenta ingresar no esta dentro del rango indicado |
| 5 | `DELETE` de `proyecto` con experimentos asociados | Va a prohibir su eliminacion teniendo en cuenta que esta con la politica restrict  |
...
|---|---|---|---|---|---|---|---|
| 1 | `INSERT` válido `cientifico_datos` + `proyecto` | SIn problema|Ejecucion sin problema | | — | | |
| 2 |  `INSERT` `experimento` con `id_proyecto=999` |error  |error de ejecucion|Error Code: 1452. Cannot add or update a child row: a foreign key constraint fails (`datalab`.`experimento`, CONSTRAINT `fk_experimento_proyecto` FOREIGN KEY (`id_proyecto`) REFERENCES `proyecto` (`id_proyecto`) ON DELETE RESTRICT)| no permite que se agreguen datos que aun no estan registrados en la otra tabla| si
| 3 | `INSERT` válido `experimento` + `modelo` |  Ejecucion sin problema | Ejecucion con problema| COdigo de error 1452  |No permite almacenar informacion sobre un id foraneo si el id no existe en la tabla principal | Si|
| 4 | `INSERT` `metrica` con `valor=1.5` | Error de ejecucion  | Error en la ejecucion |Error Code: 3819. Check constraint 'chk_valor' is violated.| `chk_metrica_valor` | Si| |
| 5 | `DELETE` `proyecto` con hijos | Error de ejecucion |Error en la ejecucion |Error Code: 1451. Cannot delete or update a parent row: a foreign key constraint fails (`datalab`.`experimento`, CONSTRAINT `fk_experimento_proyecto` FOREIGN KEY (`id_proyecto`) REFERENCES `proyecto` (`id_proyecto`) ON DELETE RESTRICT)| `fk_experimento_proyecto` | | |
| 6 | `DELETE` `modelo` con métrica |Ejecucion sin problema  | Ejecucion sin problema | — | `fk_metrica_modelo` | | |
...
- **Predicciones de Semana 3/4 que se cumplieron:** [todas]
- **Predicciones que NO se cumplieron y por qué:** [ninguna]
- **Ajustes realizados al DDL:** [Ninguno]
```
